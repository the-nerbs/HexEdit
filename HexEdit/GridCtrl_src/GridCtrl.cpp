...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    {
#if _MSC_VER >= 1300
        ValidateAndModifyCellContents(m_idCurrentCell.row, m_idCurrentCell.col, L"");
#else
...
void CGridCtrl::OnDraw(CDC* pDC)
{
    ...
    if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
    {
        ...
        for (col = minVisibleCol; col <= maxVisibleCol; col++)
        {
            ...
            if (m_nGridLines == GVL_BOTH)
                pDC->LineTo(x-1, VisRect.bottom);
            else
                pDC->LineTo(x-1, 30000);
        }
    }
                    ...
                    BOOL bForceRepaint /*=FALSE*/, BOOL bSelectCells/*=TRUE*/, BOOL bAdd /*=FALSE*/)
{
...
#else
        // Create temp set and exchange it with current selection set (automatically clears selection)
        std::set<DWORD> tmp;
        tmp.swap(m_SelMap);

        for (std::set<DWORD>::iterator pp = tmp.begin(); pp != tmp.end(); ++pp)
        {
            int row = LOWORD(*pp);
            int col = HIWORD(*pp);
            CGridCellBase* pCell;

            if (IsValid(row, col) && (pCell = GetCell(row, col)) != NULL)
            {
                // Set the cell's state not selected
                pCell->SetState(pCell->GetState() & ~GVIS_SELECTED);

                // If this is to be reselected, continue on past the redraw
                if (nMinRow <= row && row <= nMaxRow &&
                    nMinCol <= col && col <= nMaxCol)
                    continue;
                ...
                if ( VisCellRange.IsValid() && VisCellRange.InRange(row, col) )
                {
                    if (bForceRepaint && pDC)                    // Redraw NOW
                        ...
                        InvalidateCellRect(row, col);            // Redraw at leisure
                }
            }
        }
#endif
...
#else
            for (std::set<DWORD>::const_iterator pp = m_PrevSelMap.begin(); pp != m_PrevSelMap.end(); ++pp)
            {
                int row = LOWORD(*pp);
                int col = HIWORD(*pp);
                ...
                // Set state as Selected. This will add the cell to m_SelMap
        ...
        nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++)
            {
                ...
                // Set the selected state. This will add/remove the cell to m_SelMap
                if (bSelectCells)
                    SetItemState(row, col, nState | GVIS_SELECTED);
                else
    ...
    SendMessageToParent(nMinRow, nMinCol, GVN_SELCHANGED);
...
#if _MSC_VER >= 1300
void CGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCWSTR strText)
#else
...
void CGridCtrl::ClearCells(CCellRange Selection)
{
    for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
    {
        for (int col = Selection.GetMinCol(); col <= Selection.GetMaxCol(); col++)
        {
            ...
            {
#if _MSC_VER >= 1300
                ValidateAndModifyCellContents(row, col, L"");
#else
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    TRY
    {
        if (nColumn < 0)
        {
            nColumn = m_nCols;
            m_arColWidths.Add(0);
            m_col_size.insert(m_col_size.end(), -1);  // add to end
        ...
        else
        {
            ...
            m_col_size.insert(m_col_size.begin() + nColumn, -1);
    ...
    {
        for (int row = 0; row < m_nRows; ++row)
            for (int col = nColumn+1; col < m_nCols; ++col)
                GetCell(row, col)->SetCoords(row, col);
    }
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    TRY
    {
        ...
        {
            m_arRowHeights.InsertAt(nRow, 0, 1);
            if (!GetVirtualMode())
    ...
    if (!GetVirtualMode())
    {
        ...
        // Set coords for all moved cells
        for (int row = nRow+1; row < m_nRows; ++row)
            for (int col = 0; col < m_nCols; ++col)
                GetCell(row, col)->SetCoords(row, col);
    }
...
BOOL CGridCtrl::DeleteColumn(int nColumn)
{
    ...
    m_col_size.erase(m_col_size.begin() + nColumn);
    m_nCols--;
    ...
    {
        for (int row = 0; row < m_nRows; ++row)
            for (int col = nColumn; col < m_nCols; ++col)
                GetCell(row, col)->SetCoords(row, col);
    }
...
BOOL CGridCtrl::DeleteRow(int nRow)
{
    ...
    {
        // Set coords for all moved cells
        for (int row = nRow; row < m_nRows; ++row)
            for (int col = 0; col < m_nCols; ++col)
                GetCell(row, col)->SetCoords(row, col);
    }
...
#if _MSC_VER >= 1300
	return wcscmp(pCell1->GetText(), pCell2->GetText());
#else
...
#if _MSC_VER >= 1300
	int nValue1 = _wtol(pCell1->GetText());
	int nValue2 = _wtol(pCell2->GetText());
#else
...
#if _MSC_VER >= 1300
BOOL CGridCtrl::SetItem(const GV_ITEMW* pItem)
{
    if (!pItem || GetVirtualMode())
        return FALSE;
...
#if _MSC_VER >= 1300
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCWSTR strw)
{
    if (GetVirtualMode())
        return FALSE;
...
#else
        m_SelMap.erase(MAKELONG(nRow, nCol));
#endif
    }
...
#else
        m_SelMap.insert(MAKELONG(nRow, nCol));
#endif
    }
...
void CGridCtrl::ExpandColsNice(BOOL bExpandFixed /*=TRUE*/, UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    // If there are no columns to be resized just return
    if (GetColumnCount() <= 0 || !bExpandFixed && GetColumnCount() <= GetFixedColumnCount())
        return;

    CDC* pDC = GetDC();
    if (pDC == NULL)
    {
        ASSERT(0);              // we should always be able to get the DC
        return;
    }
    ...
    int last_col = -1;          // last non-hidden column
    int fixed_width = 0;        // width of columns that stay the same
    int var_width = 0;          // width of columns that can vary (but have a minimum width)
    std::vector<int> min_size = m_col_size;
    ASSERT(min_size.size() == GetColumnCount());

    // Work out the sum of the minimum sizes for all columns and
    // work out the width of columns that have not been resized.
    for (col = 0; col < GetColumnCount(); col++)
    {
        if (m_arColWidths[col] <= 0)
            ;                   // Ignore hidden columns
        else if (col < GetFixedColumnCount() && !bExpandFixed)
            fixed_width += m_arColWidths[col];
        else if (m_col_size[col] != -1)
        {
            // Column that has been resized by the use (to m_col_size[col])
            // Min width for this column (min_size[col]) has been set above to be m_col_size[col]
            var_width += m_col_size[col];
            last_col = col;
        }
        else
        {
            // Column has not been resized (m_col_size == -1) so work out based on width of contents
            int max_width = 1;  // Max width of all cells in this column (at least 1 pixel wide else it becomes hidden)
            ...
            int nStartRow = (nAutoSizeStyle & GVS_HEADER)? 0 : GetFixedRowCount();
            int nEndRow   = (nAutoSizeStyle & GVS_DATA)? GetRowCount() : GetFixedRowCount();
            ...
            for (int row = nStartRow; row < nEndRow; ++row)
            {
                ...
                if (pCell != NULL)
                {
                    CSize size = pCell->GetCellExtent(pDC);
                    if (size.cx > max_width)
                        max_width = size.cx;
                }
            }
            min_size[col] = max_width;   // Keep track of the min width for this column

            var_width += max_width;      // Update the total width
            last_col = col;
        }
    }
    ReleaseDC(pDC);
    ...
    // Work out how much we have to expand the columns
    CRect rect;
    GetClientRect(rect);

    double factor;          // How much to expand each column that can be changed

    if (fixed_width + var_width >= rect.Width())
        factor = 1.0;       // Set column widths to minimum required size
    else
        factor = double(rect.Width() - fixed_width)/var_width;  // mult. all expandable cols by this amount
...
#else
        return m_SelMap.find(MAKELONG(nRow, nCol)) != m_SelMap.end();
#endif
    }
    else
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
    ...
    TRY
    {
        ...
        for (int i = 0; i < GetRowCount(); i++)
        {
            ...
            {
                File.WriteString(CString(GetItemText(i,j)));
...
// virtual
#if _MSC_VER >= 1300
void CGridCtrl::OnEndEditCell(int nRow, int nCol, CStringW str)
{
...
// virtual
#if _MSC_VER >= 1300
BOOL CGridCtrl::ValidateEdit(int nRow, int nCol, LPCWSTR str)
#else
