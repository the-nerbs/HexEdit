...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	if (GetText()[0] != '\0')
#if _MSC_VER >= 1300
		::DrawTextW(pDC->m_hDC, GetText(), -1, rect, GetFormat() | DT_NOPREFIX);
	//else
	//	::TextOutW(pDC->m_hDC, rect.left, rect.top, GetTextW(), GetTextW().GetLength()); // this should work under 9X
#else
...
#if _MSC_VER >= 1300
BOOL CGridCellBase::ValidateEdit(LPCWSTR str)
#else
...
{
    CSize size = GetTextExtent(CString(GetText()), pDC);
    CSize ImageSize(0,0);
