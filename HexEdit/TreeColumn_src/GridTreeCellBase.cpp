                                    ...
                                    CRect aRect)    // coordinates of bounding cell rectangle
{
    ...
    CPen xPen(PS_SOLID, 1, RGB(0, 0, 0));
...
//            TryLineTo( aRect, apDC, RectBox.right - TREE_BOX_MARGIN, iYCenter);
        ...
        int pm_size = RectBox.Height()/4;        // Determines size of plus or minus sign

        apDC->SelectObject(&xPen);
        if( TryMoveTo( aRect, apDC, iXCenter - pm_size, iYCenter) )
            TryLineTo( aRect, apDC, iXCenter + pm_size + 1, iYCenter);

        if( bIsPlus)
        {
            // ... make it into a plus sign
            if( TryMoveTo( aRect, apDC, iXCenter, iYCenter - pm_size) )
                TryLineTo( aRect, apDC, iXCenter, iYCenter + pm_size + 1);
        }
