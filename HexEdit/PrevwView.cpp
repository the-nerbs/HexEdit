...
//  > zoom fit was wrong for toolbarimages.bmp
...
// Min and max zoom levels - could perhaps be calculated better based on the size of the bitmap
...
void CPrevwView::draw_bitmap(CDC* pDC)
{
	...
	int dt = int(st/zoom_) + pos_.y;
	int dr = int(sr/zoom_) + pos_.x;
	int db = int(sb/zoom_) + pos_.y;
	...
	if (::GetDeviceCaps(pDC->GetSafeHdc(), BITSPIXEL) >= 32 && FreeImage_GetBPP(pDoc->preview_dib_) >= 32)
	{
		...
		case CHECKERBOARD:
			{
				static unsigned char HatchBits[32*4] =
				{
				...
				// Create a checkerboard brush and fill the bitmap background (but not border)
				// This could be made faster by only creating the bitmap and brush once only.
				CBitmap bm;
	...
	{
		// Some of the "border" may be in the clip rect so fill with same as OnEraseBkgnd
		CRect fillRect(0, 0, rct.Width() + xoff, rct.Height() + yoff);
		BufDC.FillSolidRect(&fillRect, afxGlobalData.clrBtnFace);
